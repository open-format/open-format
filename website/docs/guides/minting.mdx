...
cosnt mintHandler = async () => await nft.mint();
...
const nft = sdk.getNFT(contractAddress);
cosnt mintWithCommissionHandler = async (address) => await nft.mintWithCommission(address);

 <button onClick={() => mintWithCommissionHandler(commissionReceiverAddress)}>Mint NFT</button>
